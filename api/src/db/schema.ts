...
export const products = pgTable("products", {
  id: uuid("id").defaultRandom().primaryKey(),
  slug: text("slug").notNull().unique(), //if name is nike shoes then slug is nike-shoes (used for navigation)
  name: text("name").notNull(),
  category: text("category").notNull().default("General"),
  description: text("description").notNull().default(""),
  priceCents: integer("price_cents").notNull(), //because using polal we need to convert cents
  currency: text("currency").notNull().default("usd"),
  imageUrl: text("image_url"),
  /** ImageKit `fileId` for deletes */
  imageKitFileId: text("image_kit_file_id"), // to delete image
  active: boolean("active").notNull().default(true), //stock or out of stock
  createdAt: timestamp("created_at", { withTimezone: true })
    .defaultNow()
    .notNull(),
});
...
export const checkoutSessions = pgTable("checkout_sessions", {
  ...
  userId: uuid("user_id")
    .notNull()
    .references(() => users.id, { onDelete: "cascade" }), //if user account is deleted then delete all checkout's
